...
from src.datasets.base_dataset import TrackingDataset, TrackingSet
from src.utils import xywh_to_ltwh
...
class SoccerNetGameState(TrackingDataset):
    def __init__(self,
        ...
        if vids_dict is None:
            vids_dict = {}

        assert self.dataset_path.exists(), (f"'{self.dataset_path}' directory does not exist. Please check the path or "
                                            f"download the dataset following the instructions here: "
                                            f"https://github.com/SoccerNet/sn-gamestate")
    ...
    def soccernet_encoding(dataframe: pd.DataFrame, supercategory):
        ...
        if supercategory == "object":
            ...
            dataframe["attributes"] = [{"role": x.get("role"), "jersey": x.get("jersey"), "team": x.get("team")} for
                                       n, x in dataframe.iterrows()]
...
def extract_category(attributes):
    if attributes['role'] == 'goalkeeper':
        ...
        category = attributes['role']
    return category


def dict_to_df_detections(annotation_dict, categories_list):
    """
    Helper function to convert annotations dictionary in Soccernet GS dataset to dataframe
    """
    log.warning(f"Debug: Annotations type: {type(annotation_dict)}")
    df = pd.DataFrame.from_dict(annotation_dict)
    log.warning(f"Debug: Annotations dict: {df.columns}")

    annotations_pitch_camera = df.loc[df['supercategory'] == 'pitch']  # remove the rows with non-human categories

    df = df.loc[df['supercategory'] == 'object']  # remove the rows with non-human categories

    df['bbox_ltwh'] = df.apply(lambda row: xywh_to_ltwh(
    ...
    df[
        'position'] = None  # df.apply(lambda row: row['attributes']['position'], axis=1) For now there is no position in the json file
    ...
    video_level_categories = list(df['category'].unique())  # e.g. player_right_17
...
def video_dir_to_dfs(args):
    """
    Load video from directory and change it to data frame structure. See Labels-GameState.json file in
    gamestate reconstruction dataset.
    """
    ...
    if os.path.isdir(video_folder_path):
        if not (Path(video_folder_path) / "Labels-GameState.json").exists():  # No labels exist for images (e.g.
            # position of bounding boxes, position of key points in the pitch, etc. )
            ...
            detections_df, annotation_pitch_camera_df, video_level_categories = dict_to_df_detections(annotations_data,
                                                                                                      categories_data)
...
def load_set(dataset_path, nvid=-1, vids_filter_set=None):
    ...
    video_list = os.listdir(dataset_path)  # List of video data to be loaded
    video_list.sort()

    # Check if videos we want to load are missing
    ...
    assert len(
        video_list) != 0, f"After applying filtering, no videos left in the '{split}' set, please fix the 'dataset.vids_dict' config."

    pool = Pool()
    args = [{"dataset_path": dataset_path, "video_folder": video_folder, "split": split} for video_folder in video_list]
    log.debug(args)
    for result in progress(pool.imap_unordered(video_dir_to_dfs, args), total=len(args),
        ...
        if result is not None:
            ...
            detections_list.append(result["detections"])  # Information on Object Detections
            annotations_pitch_camera_list.append(result["annotations_pitch_camera"])  # Information on pitch keypoints
            categories_list += result["video_level_categories"]  # List of objects in the video
    ...
    if len(categories_list) == 0:
        ...
        detections['id'] = detections['video_id'].astype(str) + "_" + detections['image_id'].astype(str) + "_" + \
        ...
        video_metadata_columns = ['name', 'nframes', 'frame_rate', 'seq_length', 'im_width', 'im_height', 'game_id',
                                  'action_position',
                                  'action_class', 'visibility', 'clip_start', 'game_time_start', 'clip_stop',
                                  'game_time_stop',
                                  'num_tracklets',
